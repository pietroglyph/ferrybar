...
func main() {
	...
	flag.StringVarP(&conf.apiKey, "key", "k", "", "WSDOT Traveller Information API key (provisioned at https://wsdot.wa.gov/traffic/api/)") // Required
	flag.IntVarP(&conf.terminal, "terminal", "t", 3,                                                                                        // 3 is the Bainbridge Island Ferry terminal
		"Terminal ID to get arriving and departing boat progress for. A terminal list is available by making a GET to https://www.wsdot.wa.gov/ferries/api/terminals/rest/terminalbasics?apiaccesscode={CODE}")
	...
	for {
		...
		for _, v := range relaventLocationData {
			if v.AtDock || !v.InService {
				...
				fmt.Print(v.VesselName+": ", big.NewFloat(v.process(&conf)*100).SetMode(big.AwayFromZero).Text('f', 4), " percent to ", v.ArrivingTerminalName, "\n")
			}
